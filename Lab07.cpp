...
int main()
{
   int angle = computeAngle(x, y);

   //cout << "ANGLE: " << angle << endl;

   double radians2 = convertToRadians(angle);

   cout << "RADIANS: " << radians2 << endl;

   double radians = 0.524;

   gHeight = gravityHeight();

   //cout << "Gravity Height: " << gHeight << endl;

   ddx = computeHorizontalComp(radians, gHeight);

   //cout << "DDX: " << ddx << endl;

   ddy = computeVerticalComp(radians, gHeight);

   cout << "DDY: " << ddy << endl;

   dx = ConstantAcceleration(dx, ddx);

   //cout << "DX: " << dx << endl;

   dy = ConstantAcceleration(dy, ddy);

   //cout << "DY: " << dy << endl;

   x = distanceFormula(x, dx, ddx);

   //cout << "X: " << x << endl;

   y = distanceFormula(y, dy, ddy);
